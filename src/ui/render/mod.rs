...
impl UiRenderer {
    ...
    pub fn draw_with_post_process(&mut self) -> Result<(), SwapchainError> {
        self.last_z = 99.0;
        self.last_texture = 0;
        self.used_textures.clear();
        let renderer2d = self.renderer2d.get_mut();
        ...
        let out = renderer2d.get_geometry_image(frame_index as usize);
        //run 2nd draw call
        renderer2d.set_texture(0, &out, SamplerType::Nearest);
        for mut tri in ctx::rectangle()
            .xywh(0, 0, renderer2d.get_extent().width as i32, renderer2d.get_extent().height as i32)
            .create()
            .triangles
        {
            tri.tex_id = Some(0);
            renderer2d.add_shape(tri);
        }
        renderer2d.draw();

        let after_post_process = renderer2d.get_geometry_image(frame_index as usize);
